...
class Net(nn.Module):
    def __init__(self, i_norm=False, noise=False):
        super(Net, self).__init__()
        self.noise = noise
        self.normalization_layer = nn.InstanceNorm2d if i_norm else nn.BatchNorm2d

        # self.block_0 = PartialConv2d(in_channels=3, out_channels=64, kernel_size=7, stride=2, padding=3, multi_channel=True, return_mask=True)
        self.block_0 = nn.Conv2d(in_channels=6, out_channels=64, kernel_size=7, stride=2, padding=3)
        # self.block_1 = PartialConv2d(in_channels=64, out_channels=128, kernel_size=5, stride=2, padding=2, multi_channel=True, return_mask=True)
        self.block_1 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=5, stride=2, padding=2)
        self.norm_1 = self.normalization_layer(num_features=128)
        # self.block_2 = PartialConv2d(in_channels=128, out_channels=256, kernel_size=5, stride=1, padding=2, multi_channel=True, return_mask=True)
        self.block_2 = nn.Conv2d(in_channels=128, out_channels=256, kernel_size=5, stride=1, padding=2)
        self.norm_2 = self.normalization_layer(num_features=256)
        ...
        self.block_3 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=5, stride=2, padding=2)
        ...
        self.dilated_block_4 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, padding=2, dilation=2, stride=1)  # decrease spatial dimension??
        ...
        self.block_6 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, padding=1)
        self.norm_6 = self.normalization_layer(num_features=128)
        # self.s_attention_7 = SelfAttention(in_channels=384)
        self.block_7 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, padding=1)
        self.norm_7 = self.normalization_layer(num_features=128)
        self.block_8 = nn.Conv2d(in_channels=192, out_channels=64, kernel_size=3, padding=1)
        self.norm_8 = self.normalization_layer(num_features=64)
        self.block_9 = nn.Conv2d(in_channels=70, out_channels=3, kernel_size=3, padding=1)
    ...
    def forward(self, x, mask=None):
        ...
        dilated_x_1 = self.dilated_block_1(x_3)
        ...
        dilated_x_2 = self.dilated_block_2(dilated_x_1)
        dilated_x_2 = torch.relu(self.dilated_norm_2(dilated_x_2))
        dilated_x_2 += dilated_x_1
        # dilated_x_3, dilated_m_3 = self.dilated_block_3(dilated_x_2, dilated_m_2)
        dilated_x_3 = self.dilated_block_3(dilated_x_2)
        dilated_x_3 = torch.relu(self.dilated_norm_3(dilated_x_3))
        dilated_x_3 += dilated_x_2
        dilated_x = torch.relu(self.dilated_norm_4(self.dilated_block_4(dilated_x_3)))
